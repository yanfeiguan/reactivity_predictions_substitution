...
data_folder = '/home/yanfeig/Projects/QM_desc/wln_QM/QM_GNN/data'

rxn_comp_df = pd.read_pickle(os.path.join(data_folder, 'regio_nonstereo_12k_QM', 'reactants.pickle.gz'))
rxn_comp_df = rxn_comp_df[~rxn_comp_df.compounds_map.duplicated()].set_index('compounds_map')
qm_descriptors_folder = '/home/yanfeig/Projects/QM_desc/wln_QM/QM_descriptors/QM_descriptors_necessary'
...
def _mol2graph(rs, idxfunc=lambda x: x.GetIdx(), core=[]):
    ...
    for smiles in rs.split('.'):
        ...
        bond_index = np.expand_dims(qm_series['bond_index_matrix'], -1)
        bond_index = np.apply_along_axis(rbf_expansion, -1, bond_index, 0.5, 0.1, 25)

        bond_distance = np.expand_dims(qm_series['distances'], -1)
        ...
        for map_idx, idx in dict.items():
            fatoms_geo[fatom_index[map_idx - 1], :] = atom_features(mol_rs.GetAtomWithIdx(fatom_index[map_idx - 1]))
            fatoms_qm[fatom_index[map_idx - 1], :] = atom_qm_descriptor[idx, :]
            if fatom_index[map_idx - 1] in core:
                core_mask[fatom_index[map_idx - 1]] = 1
...
def smiles2graph_pr(p_smiles, r_smiles, idxfunc=lambda x: x.GetIdx(), core_buffer=0):
    ...
    # p_features = _mol2graph(p, core=p_core)
